# cfn-handler-rules Specification

## Purpose

The umbrella capability for every cfn-lint rule shipped by
`cfn-lint-cfn-handler`. Each rule under this capability catches a
class of misconfiguration specific to projects using the
[`cfn-handler`](https://github.com/igorlg/cfn-handler) library for
CloudFormation Custom Resource handlers — checks cfn-lint's built-in
rules cannot make because they encode `cfn-handler`-specific
invariants (the 30-second response safety margin, the polling
re-invocation contract, the published Lambda Layer ARN manifest).

Rules in this capability share a common algorithmic shape:

1. Walk every CloudFormation custom resource in the template — both
   `AWS::CloudFormation::CustomResource` and `Custom::*` alias forms.
2. Resolve the resource's `ServiceToken` to a same-template
   `AWS::Lambda::Function` via `!Ref` or `!GetAtt`. References that
   cannot be statically resolved (literal external ARNs,
   `Fn::ImportValue`, `Fn::Sub`, parameter references) skip silently
   rather than emit false positives.
3. Apply the rule-specific check against the resolved Lambda's
   properties and/or the custom resource's properties.

Rule IDs sit in the cfn-lint community range `9000-9999` (starting at
`9100`); severity is encoded in the first letter (`E*` error, `W*`
warning, `I*` informational), per cfn-lint convention. No separate
severity configuration; users override via standard cfn-lint
mechanisms (`ignore_checks`, per-rule `configure_rules`).
